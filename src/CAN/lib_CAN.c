 ...
 *----------------------------------------------------------------------------*/
void CAN_Init (void) {
  CAN_setup(CAN_CONTROLLER_FOR_GAME);                                  /* setup CAN Controller #1 */
	
  // Enable reception for the necessary message ids
  CAN_wrFilter(CAN_CONTROLLER_FOR_GAME, 1, STANDARD_FORMAT);
	CAN_wrFilter(CAN_CONTROLLER_FOR_GAME, 2, STANDARD_FORMAT);
	CAN_wrFilter(CAN_CONTROLLER_FOR_GAME, CAN_HANSHAKE_ID, STANDARD_FORMAT);
	CAN_wrFilter(CAN_CONTROLLER_FOR_GAME, CAN_HANSHAKE_ACK_ID, STANDARD_FORMAT);
	CAN_wrFilter(CAN_CONTROLLER_FOR_GAME, CAN_BOARD_READY_ID, STANDARD_FORMAT);
	
  CAN_start(CAN_CONTROLLER_FOR_GAME);                                  /* start CAN Controller #1 */
  CAN_waitReady(CAN_CONTROLLER_FOR_GAME);                              /* wait til tx mbx is empty */
}
